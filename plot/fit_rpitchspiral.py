...
#dumpfiles = ff.find_sorted_local_input_fileset('eigenvalues*')
#dumpfiles = ['clus300', 'clus301', 'clus302','clus303']
#dumpfiles = ['clus300']
#dumpfiles = ['eigenvaluesV_main041_little']

# Get dumpfiles from spirallist.txt

#dumpfiles = np.genfromtxt('spirallist.txt',skiprows=1)

print dumpfiles, len(dumpfiles)
...
for dumpfile in dumpfiles:
	...
	rp = rpinit
		...
		rpnext = np.random.randn()*sigma_rp + rp
            ...
            chinext = get_chisquared(xi, yi, anext, d1next,d2next,rpnext, x0next, y0next, npoints, xsign=xsignnext, ysign=ysignnext,sigma=0.5)
